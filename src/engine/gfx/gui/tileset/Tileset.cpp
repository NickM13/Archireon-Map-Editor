#include "engine\gfx\gui\tileset\Tileset.h"
...
void CTileSet::input(Sint8& p_interactFlags, Sint8* p_keyStates, Sint8* p_mouseStates, Vector2<Sint32> p_mousePos)
{
	...
	if(p_mousePos.x >= m_pos.x - (m_borderThickness - 1) && p_mousePos.x <= m_pos.x + _size.x + (m_borderThickness - 1) &&
		p_mousePos.y >= m_pos.y - 20 - (m_borderThickness - 1) && p_mousePos.y <= m_pos.y + _size.y + (m_borderThickness - 1))
		m_hover = true;
	else
		m_hover = false;
	if(((p_interactFlags & 1) == 0) && p_mouseStates[0] == 1 &&
		p_mousePos.x >= m_pos.x && p_mousePos.x <= m_pos.x + _size.x &&
		p_mousePos.y >= m_pos.y - 20 && p_mousePos.y <= m_pos.y)
		{
			//m_selected = Sint8(!m_selected);
		}
	...
	if(p_keyStates[GLFW_KEY_LEFT] == 1)
	{
		m_selectedTile.x--;
	...
	if(p_keyStates[GLFW_KEY_RIGHT] == 1)
	{
		m_selectedTile.x++;
	...
	if(p_keyStates[GLFW_KEY_DOWN] == 1)
	{
		m_selectedTile.y++;
...
void CTileSet::render()
{
	...
	glPushMatrix();
	{
		...
		if(m_selected == 0)
			MScissor::getInstance().push(Rect(-GLfloat(m_borderThickness), -GLfloat(m_borderThickness + 20), GLfloat(_size.x + m_borderThickness * 2), GLfloat(_size.y + m_borderThickness * 2 + 20)));
		else
		...
		glBegin(GL_QUADS);
		{
			...
			glVertex2f(GLfloat(_size.x + m_borderThickness), -GLfloat(m_borderThickness) - 20);
			glVertex2f(GLfloat(_size.x + m_borderThickness), GLfloat(_size.y + m_borderThickness));
			glVertex2f(-GLfloat(m_borderThickness), GLfloat(_size.y + m_borderThickness));
		}
		glEnd();
		...
		glBegin(GL_QUADS);
		{
			glVertex2f(-GLfloat(m_borderThickness - 1), GLfloat(_size.y + (m_borderThickness - 1)));
			glVertex2f(GLfloat(_size.x + (m_borderThickness - 1)), GLfloat(_size.y + (m_borderThickness - 1)));
			glVertex2f(GLfloat(_size.x + (m_borderThickness - 1)), -20 - GLfloat(m_borderThickness - 1));
		...
		glPushMatrix();
		{
			...
			{
				glTexCoord2f(0, 0);
				glVertex2f(0, m_tileSheet.getSize().y);
				glTexCoord2f(1, 0);
				glVertex2f(m_tileSheet.getSize().x, m_tileSheet.getSize().y);
				glTexCoord2f(1, 1);
				glVertex2f(m_tileSheet.getSize().x, 0);
				glTexCoord2f(0, 1);
				glVertex2f(0, 0);
			}
			glEnd();
